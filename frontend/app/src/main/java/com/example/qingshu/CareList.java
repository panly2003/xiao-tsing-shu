...
public class CareList extends AppCompatActivity {
    ...
    private List<Person> followingList = new ArrayList<>();
                ...
                try {
                    if(jsonObject.getString("status").equals("success")){
                        ...
                        for(int i = 0; i < following.size(); ++i){
                            String user_id = following.getJSONObject(i).getString("user_id");
                            String user_name = following.getJSONObject(i).getString("user_name");
                            String user_avatar = following.getJSONObject(i).getString("user_avatar");
                            runOnUiThread(() -> {
                                followingList.add(new Person(user_id, user_name, user_avatar));
                            });
                        }
                        runOnUiThread(() -> {
                            PersonAdapter adapter = new PersonAdapter(followingList);
